...
class DocxController {
    public function generateContract($id) {
        ...
        // EVALUACIÓN DE SUBSIDIO / DESCUENTO POR LEY
        $costoOriginal = $plan['costo'] ?? 0;
        $costoFinal = $costoOriginal;
        $leyendaDescuento = "";

        if (strtolower($cliente['discapacidad']) === 'si') {
            $costoFinal = $costoOriginal * 0.50; 
            $leyendaDescuento = " (Subsidio Discapacidad -50%)";
        }
        ...
        $templateProcessor->setValues([
            ...
            'megas'          => $plan['megas'] ?? '0',
            'costo'          => number_format($costoFinal, 2) . $leyendaDescuento,
